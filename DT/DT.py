...
data = pd.read_csv('C:\\Users\\wxy\\PycharmProjects\\project108\\DM\\adult_fin.csv')
...
for train_index, val_index in cv.split(X_training, y_training):
    ...
    model = DecisionTreeClassifier(random_state=i, max_depth=3)
    ...
    ax = plt.gca()
    val_roc = plot_roc_curve(model, X_val, y_val, ax=ax,lw=1, alpha=0.8)
...
print("=========================預測結果=========================")
print(" fold", foldNum)
model = DecisionTreeClassifier(random_state=bestGroup, max_depth=3)
